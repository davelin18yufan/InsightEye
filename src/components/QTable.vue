<template>
  ...
  <!-- search -->
    <template v-slot:top-right>
      <q-input borderless dense debounce="300" v-model="filter" placeholder="Search" class="input">
  ...
  setup(props, { emit }) {
    ...
    function onSelectChange() {
      ...
      const existingRow = rows.value.filter(row => 
        selected.value.some(s => Object.keys(row).every(key => row[key] === s[key]))
      )
    ...
    function filterAndSort(startRow, count, filter, sortBy, descending) {
      const data = filter
        ? rows.value.filter(row => ["name", 'cellphone', 'email', 'gender'].some(key => row[key].includes(filter)))
        : rows.value.slice()
    ...
    function onRequest(props) {
      ...
      setTimeout(() => {
        ...
        // re-format
        const formatData = filterAndSort(startRow, fetchCount, filter, sortBy, descending)
